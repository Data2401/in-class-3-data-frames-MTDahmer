...
```{r}
categoryname <- c(rownames(US_spend))
categoryname
US_spend$Category <- categoryname
...
```{r}
library(tidyr)
#US_spend[nrow(US_spend) + 1,] <- c(1,2,3)
US_spend
columnnames <- toString(colnames(US_spend))
pivot_longer(data = US_spend, cols = columnnames)
